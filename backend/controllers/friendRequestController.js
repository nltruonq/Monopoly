...
const FriendRequestController = {
    ...
    getFriendRequestsSend: async (req, res) => {
        try {
            const { username } = req.params;
            const friendRequests = await FriendRequest.find({ sender: username });

            for (let i = 0; i < friendRequests.length; i++) {
                const user = await User.findOne({ username: friendRequests[i].receiver });
                friendRequests[i] = { ...friendRequests[i]._doc, user };
            }
